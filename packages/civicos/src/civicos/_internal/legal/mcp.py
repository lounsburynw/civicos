...
Usage:
    # Run server
    civicos-legal-server

    # Or with HTTP transport
    civicos-legal-server --transport http --port 8003
"""
...
def create_mcp_server(
    ...
    if not MCP_AVAILABLE:
        raise ImportError(
            "MCP not installed. Install with: pip install civicos-legal[mcp]"
        )

    server = Server("civicos-legal")
    ...
    async def call_tool(name: str, arguments: dict):
        ...
        if name == "search_legislation":
            ...
            try:
                if mode == "keyword":
                    from civicos._internal.legal.enrichment import LegislativeCache, create_default_cache
...
def main():
    ...
    if args.transport == "http":
        ...
        try:
            ...
            print("HTTP transport requires: pip install civicos-legal[http]")
            return 1
    else:
