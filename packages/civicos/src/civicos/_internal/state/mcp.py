...
Exposes StateManager queries as MCP tools. Part of civicos-state package.
...
CLI:
    civicos-state-server [--http] [--port PORT] [--db PATH]
"""
...
def create_mcp_server(db_path: str = "data/civic_state.db") -> IssuesServer:
    ...
    if not MCP_AVAILABLE:
        raise ImportError("MCP not installed. Install with: pip install civicos-state[mcp]")
...
def main():
    """CLI entry point for civicos-state MCP server."""
    import argparse
    import sys
    ...
    if args.http:
        try:
            ...
            print("uvicorn not installed. Install with: pip install civicos-state[http]", file=sys.stderr)
            sys.exit(1)
