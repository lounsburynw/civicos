...
Environment Variables:
    CIVICOS_DATA_ROOT: Single root for all data (development default: "data")
    CIVICOS_BUNDLED_DATA_DIR: Read-only reference data (production: /app/bundled-data)
    CIVICOS_USER_DATA_DIR: Persistent user data (production: /app/user-data)

Priority (highest to lowest):
    1. CIVICOS_BUNDLED_DATA_DIR / CIVICOS_USER_DATA_DIR (production separation)
    2. CIVICOS_DATA_ROOT (unified directory for development)
    3. Default: "data" relative to working directory

Usage:
    from civicos.paths import get_data_path, get_state_db_path, get_vectors_dir
...
class DataPathResolver:
    ...
    - Development: All data in single directory (CIVICOS_DATA_ROOT or "data")
    ...
    def __init__(
        ...
        If no args provided, reads from environment:
            - CIVICOS_BUNDLED_DATA_DIR for bundled data
            - CIVICOS_USER_DATA_DIR for user data
            - CIVICOS_DATA_ROOT as fallback for both
        ...
        if root is not None:
            ...
            env_bundled = os.environ.get("CIVICOS_BUNDLED_DATA_DIR")
            env_user = os.environ.get("CIVICOS_USER_DATA_DIR")
            env_root = os.environ.get("CIVICOS_DATA_ROOT", "data")
