...
Tests cover:
- Default path resolution (CIVICOS_DATA_ROOT fallback)
...
class TestDataPathResolver:
    ...
    def test_default_root_is_data(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            for key in ["CIVICOS_DATA_ROOT", "CIVICOS_BUNDLED_DATA_DIR", "CIVICOS_USER_DATA_DIR"]:
    ...
    def test_explicit_root_overrides_env(self):
        ...
        with patch.dict(os.environ, {"CIVICOS_DATA_ROOT": "/env/path"}):
    ...
    def test_civic_data_root_env_var(self):
        """CIVICOS_DATA_ROOT should set both bundled and user roots."""
        with patch.dict(os.environ, {"CIVICOS_DATA_ROOT": "/custom/data"}, clear=True):
            for key in ["CIVICOS_BUNDLED_DATA_DIR", "CIVICOS_USER_DATA_DIR"]:
                os.environ.pop(key, None)
            resolver = DataPathResolver()
    ...
    def test_separate_bundled_and_user_dirs(self):
        ...
        with patch.dict(os.environ, {
            "CIVICOS_BUNDLED_DATA_DIR": "/app/bundled-data",
            "CIVICOS_USER_DATA_DIR": "/app/user-data",
        }):
            resolver = DataPathResolver()
    ...
    def test_bundled_dir_overrides_root(self):
        """CIVICOS_BUNDLED_DATA_DIR should override CIVICOS_DATA_ROOT for bundled."""
        with patch.dict(os.environ, {
            "CIVICOS_DATA_ROOT": "/fallback",
            "CIVICOS_BUNDLED_DATA_DIR": "/app/bundled",
        }):
            os.environ.pop("CIVICOS_USER_DATA_DIR", None)
            resolver = DataPathResolver()
            assert resolver.bundled_root == Path("/app/bundled")
            assert resolver.user_root == Path("/fallback")  # Falls back to CIVICOS_DATA_ROOT

    def test_user_dir_overrides_root(self):
        """CIVICOS_USER_DATA_DIR should override CIVICOS_DATA_ROOT for user data."""
        with patch.dict(os.environ, {
            "CIVICOS_DATA_ROOT": "/fallback",
            "CIVICOS_USER_DATA_DIR": "/app/user",
        }):
            os.environ.pop("CIVICOS_BUNDLED_DATA_DIR", None)
            resolver = DataPathResolver()
...
class TestModuleFunctions:
    ...
    def test_get_resolver_returns_singleton(self):
        ...
        with patch.dict(os.environ, {"CIVICOS_DATA_ROOT": "/singleton/test"}):
    ...
    def test_reset_resolver_clears_singleton(self):
        ...
        with patch.dict(os.environ, {"CIVICOS_DATA_ROOT": "/first"}):
            resolver1 = get_resolver()
            reset_resolver()

        with patch.dict(os.environ, {"CIVICOS_DATA_ROOT": "/second"}):
            resolver2 = get_resolver()
...
class TestProductionConfiguration:
    ...
    def test_development_unified_directory(self):
        ...
        with patch.dict(os.environ, {"CIVICOS_DATA_ROOT": "data"}, clear=True):
            for key in ["CIVICOS_BUNDLED_DATA_DIR", "CIVICOS_USER_DATA_DIR"]:
                os.environ.pop(key, None)
            reset_resolver()
...
class TestEmbeddingModelConfiguration:
    """Tests for CIVICOS_EMBEDDING_MODEL environment variable support."""

    def test_default_model_without_env(self):
        """Without CIVICOS_EMBEDDING_MODEL, should use nomic default."""
        import sys
        import importlib
    ...
    def test_custom_model_from_env(self):
        """CIVICOS_EMBEDDING_MODEL should override default embedding model."""
        import sys
        import importlib
