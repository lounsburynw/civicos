...
class CivicConfig:
    ...
    def _detect_environment(self) -> str:
        ...
        env = os.getenv('CIVICOS_ENV', 'development').lower()
    ...
    def get_api_keys(self) -> Dict[str, str]:
        ...
        # Production keys
        if os.getenv('CIVICOS_WEB_KEY'):
            keys[os.getenv('CIVICOS_WEB_KEY')] = 'web_interface'
        
        if os.getenv('CIVICOS_DEMO_KEY'):
            keys[os.getenv('CIVICOS_DEMO_KEY')] = 'demo_user'
        
        if os.getenv('CIVICOS_TEST_KEY'):
            keys[os.getenv('CIVICOS_TEST_KEY')] = 'test_user'
        ...
        if self.env in ['production', 'staging'] and not keys:
            raise RuntimeError(f"No API keys configured for {self.env} environment. Set CIVICOS_WEB_KEY environment variable.")
    ...
    def get_bundled_data_dir(self) -> Path:
        ...
        In development: data/
        """
        data_dir = os.getenv('CIVICOS_BUNDLED_DATA_DIR')
        if data_dir:
    ...
    def get_user_data_dir(self) -> Path:
        ...
        In development: data/
        """
        data_dir = os.getenv('CIVICOS_USER_DATA_DIR')
        if data_dir:
            return Path(data_dir)
    ...
    def get_api_port(self) -> int:
        ...
        port = os.getenv('CIVICOS_API_PORT')
        if port:
            try:
                return int(port)
            except ValueError:
                print(f"⚠️  Invalid CIVICOS_API_PORT '{port}', using default")
    ...
    def get_api_endpoint(self) -> str:
        ...
        endpoints = {
            'staging': os.getenv('CIVICOS_API_STAGING', 'https://staging.civic.example.com'),
            'production': os.getenv('CIVICOS_API_PROD', 'https://api.civic.example.com')
        }
    ...
    def get_cors_origins(self) -> list:
        ...
        origins = os.getenv('CIVICOS_CORS_ORIGINS', '').split(',')
    ...
    def _validate_core_config(self, result: 'ValidationResult') -> None:
        ...
        # CIVICOS_ENV validation
        env = os.getenv('CIVICOS_ENV', '')
        if not env:
            result.warnings.append("CIVICOS_ENV not set, defaulting to 'development'")
            result.suggestions['CIVICOS_ENV'] = "Set CIVICOS_ENV=development|staging|production"
        elif env not in ['development', 'staging', 'production']:
            result.warnings.append(f"CIVICOS_ENV='{env}' is non-standard, using 'development'")
            result.suggestions['CIVICOS_ENV'] = "Valid values: development, staging, production"
        ...
        if port:
            try:
                ...
                if not (1024 <= port_int <= 65535):
                    result.warnings.append(f"CIVICOS_API_PORT={port} outside recommended range (1024-65535)")
            except ValueError:
                result.errors.append(f"CIVICOS_API_PORT='{port}' is not a valid integer")
                result.suggestions['CIVICOS_API_PORT'] = "Set to a valid port number (e.g., 8001)"

    def _validate_security_config(self, result: 'ValidationResult') -> None:
        """Validate authentication and security configuration."""
        has_web_key = bool(os.getenv('CIVICOS_WEB_KEY'))
        has_demo_key = bool(os.getenv('CIVICOS_DEMO_KEY'))
        has_dev_mode = os.getenv('CIVICOS_DEV_MODE', '').lower() == 'true'

        if self.env in ['production', 'staging']:
            # Production/staging REQUIRE proper authentication
            if not has_web_key:
                result.errors.append("CIVICOS_WEB_KEY is required for authentication in production/staging")
                result.suggestions['CIVICOS_WEB_KEY'] = "Generate with: openssl rand -hex 32"
            elif os.getenv('CIVICOS_WEB_KEY') == 'dev_key_local':
                result.errors.append("CIVICOS_WEB_KEY cannot be 'dev_key_local' in production/staging")
                result.suggestions['CIVICOS_WEB_KEY'] = "Generate a secure key: openssl rand -hex 32"

            # CORS must be configured
            cors = os.getenv('CIVICOS_CORS_ORIGINS', '')
            if not cors:
                result.errors.append("CIVICOS_CORS_ORIGINS must be set to whitelist allowed domains")
                result.suggestions['CIVICOS_CORS_ORIGINS'] = "Example: https://your-domain.com,https://staging.your-domain.com"
            elif '*' in cors:
                result.errors.append("CIVICOS_CORS_ORIGINS cannot contain '*' in production/staging")

        elif self.env == 'development':
            # Development needs at least one auth method
            if not any([has_web_key, has_demo_key, has_dev_mode]):
                result.errors.append("Development requires CIVICOS_WEB_KEY, CIVICOS_DEMO_KEY, or CIVICOS_DEV_MODE=true")
                result.suggestions['CIVICOS_DEV_MODE'] = "Set CIVICOS_DEV_MODE=true for local development"
    ...
    def _validate_data_paths(self, result: 'ValidationResult') -> None:
        ...
        if not bundled_dir.exists():
            if self.env == 'production':
                ...
                result.suggestions['CIVICOS_BUNDLED_DATA_DIR'] = "Ensure bundled data is deployed to /app/bundled-data"
            else:
        ...
        if self.env == 'production':
            if not user_dir.exists():
                ...
                result.suggestions['CIVICOS_USER_DATA_DIR'] = "Mount persistent volume to /app/user-data"
