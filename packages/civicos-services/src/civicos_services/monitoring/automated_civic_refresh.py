...
from civicos_config import CITY_CONFIGS
...
class ProductionAlertManager:
    ...
    def __init__(self):
        self.alert_recipients = os.getenv('CIVICOS_ALERT_EMAILS', '').split(',')
    ...
    def send_budget_alert(self, budget_status: dict):
        ...
        if not self.alert_recipients or not self.smtp_username:
            print("⚠️  Email alerting not configured (missing CIVICOS_ALERT_EMAILS or SMTP credentials)")
            return
