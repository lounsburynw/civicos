...
class AgendaIntegrator:
    ...
    def __init__(
        ...
        Args:
            provider: LLM provider instance (required). Get from civic_services.core.llm_provider.
                     Example: get_model_for_task('long_document') for Gemini 1.5 Pro with 2M context
            model_name: Model name for cost tracking. If not provided, uses provider.default_model.
            cost_calculator: Optional callable(model_name, usage_dict) -> float for cost tracking.
                            If not provided, cost tracking is disabled.
        """
        # Dependency injection: provider MUST be passed
        # CLI entry points (in civic_extraction/cli/) get provider from civic_services
        if provider is not None:
            self.provider = provider
            self._model_name = model_name or getattr(provider, 'default_model', 'unknown')
        else:
            raise ValueError(
                "AgendaIntegrator requires a provider argument. "
                "Use: AgendaIntegrator(provider=get_model_for_task('long_document')) "
                "where get_model_for_task is imported from civic_services.core.llm_provider"
            )
...
Skip only purely procedural items (meeting minutes approval, internal appointments)."""
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 1500) -> str:
        ...
        try:
            ...
            # Track cost using injected cost calculator (if provided)
            if response.usage:
                if self._cost_calculator:
                    call_cost = self._cost_calculator(self._model_name, response.usage)
