...
from civic_services.core.config import CivicConfig, ValidationResult, validate_config
...
class TestValidationResult:
    ...
    def test_format_errors_with_all_sections(self):
        ...
        result = ValidationResult(
            ...
            errors=["Missing CIVIC_WEB_KEY", "Missing OPENAI_API_KEY"],
            warnings=["CIVIC_ENV not set"],
            suggestions={"CIVIC_WEB_KEY": "Generate with: openssl rand -hex 32"},
        )
        formatted = result.format_errors()

        assert "production" in formatted
        assert "ERRORS (must fix):" in formatted
        assert "Missing CIVIC_WEB_KEY" in formatted
        assert "Missing OPENAI_API_KEY" in formatted
        assert "WARNINGS (should address):" in formatted
        assert "CIVIC_ENV not set" in formatted
...
class TestCoreConfigValidation:
    ...
    def test_validates_civic_env_warning_when_missing(self):
        """Should warn when CIVIC_ENV is not set."""
        with patch.dict(os.environ, {}, clear=True):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)

        # Check for the warning about CIVIC_ENV
        env_warnings = [w for w in result.warnings if "CIVIC_ENV" in w]
        assert len(env_warnings) >= 1

    def test_validates_civic_env_warning_for_invalid_value(self):
        """Should warn when CIVIC_ENV has invalid value."""
        with patch.dict(os.environ, {"CIVIC_ENV": "invalid_env"}, clear=True):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)
    ...
    def test_validates_port_invalid_string(self):
        """Should error when CIVIC_API_PORT is not a valid integer."""
        with patch.dict(
            os.environ,
            {"CIVIC_ENV": "development", "CIVIC_DEV_MODE": "true", "CIVIC_API_PORT": "not_a_number"},
            clear=True,
        ):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)

        port_errors = [e for e in result.errors if "CIVIC_API_PORT" in e]
...
class TestSecurityConfigValidation:
    ...
    def test_development_requires_auth_method(self):
        ...
        with patch.dict(os.environ, {"CIVIC_ENV": "development"}, clear=True):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)
        ...
        auth_errors = [
            e for e in result.errors if "CIVIC_WEB_KEY" in e or "CIVIC_DEV_MODE" in e
        ]
        assert len(auth_errors) >= 1

    def test_development_passes_with_dev_mode(self):
        """Development should pass with CIVIC_DEV_MODE=true."""
        with patch.dict(
            os.environ, {"CIVIC_ENV": "development", "CIVIC_DEV_MODE": "true"}, clear=True
        ):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)
    ...
    def test_development_passes_with_web_key(self):
        """Development should pass with CIVIC_WEB_KEY set."""
        with patch.dict(
            os.environ, {"CIVIC_ENV": "development", "CIVIC_WEB_KEY": "dev_key_local"}, clear=True
        ):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)
    ...
    def test_production_requires_web_key(self):
        """Production environment requires CIVIC_WEB_KEY."""
        with patch.dict(os.environ, {"CIVIC_ENV": "production"}, clear=True):
            config = CivicConfig()
            result = config.validate_environment(raise_on_error=False)

        assert not result.passed
        key_errors = [e for e in result.errors if "CIVIC_WEB_KEY" in e]
        assert len(key_errors) >= 1

    def test_production_rejects_dev_key(self):
        """Production should reject dev_key_local as CIVIC_WEB_KEY."""
        with patch.dict(
            os.environ,
            {
                "CIVIC_ENV": "production",
                "CIVIC_WEB_KEY": "dev_key_local",
                "CIVIC_CORS_ORIGINS": "https://example.com",
    ...
    def test_production_requires_cors_origins(self):
        """Production environment requires CIVIC_CORS_ORIGINS."""
        with patch.dict(
            os.environ,
            {
                "CIVIC_ENV": "production",
                "CIVIC_WEB_KEY": "secure_key_12345678901234567890123456",
                "OPENAI_API_KEY": "sk-real-key-here-12345678901234567890",
            },
            clear=True,
        ):
        ...
        cors_errors = [e for e in result.errors if "CIVIC_CORS_ORIGINS" in e]
    ...
    def test_passes_for_valid_production_config(self):
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "CIVIC_CORS_ORIGINS": "https://example.com",
                "OPENAI_API_KEY": "sk-real-key-here-12345678901234567890",
                "CIVIC_BUNDLED_DATA_DIR": "/tmp",  # Exists for test
                "CIVIC_USER_DATA_DIR": "/tmp",  # Exists for test
            },
            clear=True,
        ):
            config = CivicConfig()
...
class TestValidateConfigFunction:
    ...
    def test_returns_validation_result(self):
        ...
        with patch.dict(
            ...
            from civic_services.core import config as config_module
    ...
    def test_production_errors_on_missing_bundled_dir(self):
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "CIVIC_CORS_ORIGINS": "https://example.com",
                "OPENAI_API_KEY": "sk-real-key-here-12345678901234567890",
                "CIVIC_BUNDLED_DATA_DIR": "/nonexistent/path/bundled",
                "CIVIC_USER_DATA_DIR": "/tmp",
            },
            clear=True,
        ):
            config = CivicConfig()
