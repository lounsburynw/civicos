...
# GPU-enabled image for embedding workers. Uses fastembed-gpu which bundles
# onnxruntime-gpu for CUDA acceleration. Nomic on A10G is 50-100x faster than
# the CPU path, making large corpora (CA legislation, federal bills) tractable.
#
# fastembed-gpu requires CUDA runtime + CuDNN system libraries — pip alone is
# not enough. Use the official NVIDIA CUDA runtime base image which provides
# both. fastembed-gpu also requires explicit providers=["CUDAExecutionProvider"]
...
civic_gpu_image = (
    ...
    .add_local_python_source("civicos", "civicos_config")
...
@app.function(
    image=civic_gpu_image,
    secrets=[modal.Secret.from_name("civic-db")],
    gpu="A10G",  # ~$0.60/hr — Nomic on A10G is 50-100x faster than CPU
    memory=8192,  # 8GB CPU RAM — A10G has 24GB VRAM separately
    timeout=1800,
...
def _embed_legislation_page(
    ...
    try:
        ...
        with _stage("gpu_model_load"):
            # Load nomic on GPU directly. fastembed-gpu requires explicit
            # CUDAExecutionProvider — it does not auto-detect CUDA.
            gpu_model = TextEmbedding(
                model_name="nomic-ai/nomic-embed-text-v1.5",
                providers=["CUDAExecutionProvider"],
            )

        with _stage("pgvector_init"):
            # Skip PgVectorBackend's lazy provider init by attaching a stub.
            # bulk_insert_embeddings reads self._embedding_model (the model name
            # string) but doesn't actually call .encode() — embeddings are
            # generated by gpu_model above and passed in as records.
            class _GpuProviderStub:
                model_name = "nomic-ai/nomic-embed-text-v1.5"
                embedding_dimension = 768
                def encode(self_, texts, batch_size=100):
                    raise RuntimeError("Stub provider — use gpu_model directly")

            pgvector = PgVectorBackend(connection_string=database_url, provider_type="fastembed")
            pgvector._provider = _GpuProviderStub()
        ...
        # Embed and store in sub-batches. GPU can handle much larger batches
        # than CPU since the bottleneck is GPU memory not host RAM.
        EMBED_BATCH = 512
        total_success = 0
        total_failed = 0
        num_sub_batches = 0
