...
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "packages" / "civic" / "src"))
...
try:
    civic_client = Civic(DEFAULT_JURISDICTION)
    logger.info(f"Civic client initialized for {DEFAULT_JURISDICTION} (storage: {type(civic_client._storage).__name__})")
...
def search_regulatory_stack(
    ...
    try:
        ...
        stack = civic_client.what_applies(sanitized_topic)
...
def search_meeting_history(
    ...
    try:
        ...
        decisions = civic_client.what_happened(sanitized_query)
        ...
        if include_transcripts:
            ...
            excerpts = civic_client.what_was_said(sanitized_query, top_k=limit)
...
def find_similar_issues(
    ...
    try:
        ...
        community = civic_client.whos_with_me(sanitized_topic, semantic=semantic)
...
def search_agenda_packets(
    ...
    try:
        ...
        results = civic_client.what_happened_with_discussion(
            sanitized_query,
            top_k=limit,
...
def search_budget(
    ...
    try:
        ...
        # Get summary first
        summary = civic_client.budget_summary(fiscal_year=fiscal_year)
        if summary:
...
def get_upcoming_meetings(
    ...
    try:
        results = civic_client.whats_next(
            topics=topic_list,
            days=days,
...
def get_voting_record(
    ...
    try:
        record = civic_client.get_voting_record(
...
def get_decision_context(
    ...
    try:
        results = civic_client.what_happened_full_context(
            sanitized_query,
            top_k=limit,
        )
...
def get_public_testimony(
    ...
    try:
        testimonies = civic_client.get_public_testimony(sanitized_topic, top_k=limit)
...
def get_funding_flow(
    ...
    try:
        flows = civic_client.funding_flow(
            program=program,
...
def get_federal_expenditures(
    ...
    try:
        # Get summary first
        summary = civic_client.federal_expenditures_summary(audit_year=audit_year)
...
def get_intergovernmental_revenue(
    ...
    try:
        summary = civic_client.intergovernmental_revenue(
            fiscal_year=fiscal_year,
            source=source,
        )
...
def prepare_for_meeting(
    ...
    try:
        prep = civic_client.prepare(sanitized_id)
...
@mcp.resource("civic://san-rafael/meetings")
def list_meetings() -> str:
    ...
    try:
        ...
        meetings = civic_client._storage.get_meetings(civic_client.jurisdiction, limit=20)
...
@mcp.resource("civic://san-rafael/decisions")
def list_decisions() -> str:
    ...
    try:
        decisions = civic_client._storage.get_decisions(civic_client.jurisdiction, limit=20)
...
@mcp.resource("civic://san-rafael/budget-departments")
def list_budget_departments() -> str:
    ...
    try:
        summary = civic_client.budget_summary()
...
@mcp.resource("civic://san-rafael/corpus-stats")
def get_corpus_stats() -> str:
    ...
    try:
        ...
        jurisdiction = civic_client.jurisdiction
