"""
Modal deployment for Federal MCP Server.

Reference implementation for federal-level data (FAC, USAspending, Congress).
Part of the federated MCP architecture.

Tools (3):
    - get_federal_expenditures: FAC Single Audit data
    - get_funding_flow: Federal funding traces
    - search_regulatory_stack: Federal legislation

Endpoint:
    federal.civicosproject.org/mcp

Deploy:
    modal deploy apps/civicos-mcp/modal_federal.py
"""

import modal

# Import shared infrastructure (deployed via add_local_dir)
# The import happens at runtime on Modal containers
app = modal.App("civicos-federal")

# Build image with all MCP dependencies (same as modal_base.py)
mcp_image = (
...
@app.cls(
    image=mcp_image,
    secrets=[modal.Secret.from_name("civicos-federal-env")],
    memory=4096,
    timeout=300,
    min_containers=0,  # Reference implementation - no min containers
)
@modal.concurrent(max_inputs=20)
class FederalMCPServer:
    """Federal-level MCP server (3 tools)."""
    ...
    def initialize(self):
        ...
        self.logger = logging.getLogger("civicos-mcp-federal")

        # Add paths for imports
        sys.path.insert(0, "/app")
        sys.path.insert(0, "/app/civicos-mcp")

        self.logger.info("Initializing CivicOS Federal MCP Server on Modal...")

        # Import CivicOS and initialize
        from civicos import CivicOS
        from handlers.loader import load_jurisdiction_config

        self.jurisdiction = os.getenv("CIVICOS_JURISDICTION", "country-united-states")
    ...
    def _bind_handlers(self, handlers):
        """Bind handler functions for federal-level tools only."""
        enabled_tools = self.jurisdiction_config.get_enabled_tools()
        self.logger.info(f"Enabled tools for federal level: {len(enabled_tools)}")
            ...
            if method == "initialize":
                return {
                    ...
                    "result": {
                        ...
                        "serverInfo": {
                            "name": f"CivicOS Federal MCP Server",
                            "version": "1.0.0",
                        },
                        "capabilities": {
...
def main():
    """Local entrypoint."""
    print("CivicOS Federal MCP Server")
    print()
    print("Tools: get_federal_expenditures, get_funding_flow, search_regulatory_stack")
    print()
    print("Deploy: modal deploy apps/civicos-mcp/modal_federal.py")
    print("Endpoint: federal.civicosproject.org/mcp")
