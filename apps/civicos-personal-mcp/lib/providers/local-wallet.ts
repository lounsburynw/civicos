 ...
 * Encrypted keys stored in IndexedDB (browser) or filesystem (Node.js).
 *
 * Security model:
...
const STORAGE_KEY = 'civicos-personal-mcp-identity';
 ...
 * - IndexedDB storage (browser) with fallback
 */
export class LocalWalletProvider implements SigningProvider {
  ...
  async isAvailable(): Promise<boolean> {
    // LocalWallet is available if we have Web Crypto API
  ...
  async createIdentity(options: CreateIdentityOptions): Promise<{
    ...
    // Derive private key using NIP-06 path
    ...
    this.setUnlockTimeout(5 * 60 * 1000); // 5 minutes default
    ...
    return {
      ...
      mnemonic, // CRITICAL: User must back this up!
    };
  }
...
export class IndexedDBStorage implements WalletStorage {
  private dbName = 'civicos-personal-mcp';
